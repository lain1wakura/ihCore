...
class CharacterManager(private val server: Ihcore) {
    ...
    fun applyCharacter(player: ServerPlayerEntity, character: Character) {
        ...
        server.playerManager.addCharacter(character)
    ...
    fun saveAllCharacters() {
        getAllCharacters().forEach { character -> server.userService.saveOrUpdateCharacterForUser(character.author, character) }
    }
